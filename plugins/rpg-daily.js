...
let exp = `${pickRandom([500000, 600000, 700000, 800000, 900000, 900099, 1000000, 1300000, 1500000, 1800000])}` * 1
let exppremium = `${pickRandom([1000000, 1500000, 1800000, 2100000, 2500000, 2900000, 3300000, 3600000, 4000000, 4500000])}` * 1
...
let time = user.lastclaim + 7200 //2 Horas 7200000
if (new Date - user.lastclaim < 7200) return await conn.sendButton(m.chat, `𝙔𝘼 𝙍𝙀𝘾𝙇𝘼𝙈𝘼𝙎𝙏𝙀 𝙏𝙐 𝙍𝙀𝙂𝘼𝙇𝙊 🎁\n𝙑𝙐𝙀𝙇𝙑𝙀 𝙀𝙉 *${msToTime(time - new Date())}* 𝙋𝘼𝙍𝘼 𝙑𝙊𝙇𝙑𝙀𝙍 𝘼 𝙍𝙀𝘾𝙇𝘼𝙈𝘼𝙍`, wm, null, [['𝗠 𝗘 𝗡 𝗨 ☘️', '/menu']], fkontak, m)
let texto = ''
