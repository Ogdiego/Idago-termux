import fetch from 'node-fetch' 
 ...
 if (!aa) return await conn.sendFile(m.chat, json.result.audio, '.mp3', null, m, false, { mimetype: 'audio/mp4' }) 
 } catch {   
 ...
 conn.sendMessage(m.chat, { audio: { url: audiocore }, fileName: `.mp3`, mimetype: 'audio/mp4' }, { quoted: m })}} 
 if (command == 'play.2') { 
 conn.reply(m.chat, `*_⏳Sᴇ ᴇsᴛᴀ ᴘʀᴏᴄᴇsᴀɴᴅᴏ Sᴜ ᴠɪᴅᴇᴏ...⏳_*`, m)     
 try {    
 let mediaa = await ytPlayVid(text) 
 await conn.sendMessage(m.chat, { video: { url: mediaa.result }, fileName: `.mp4`, caption: `_𝐈𝐃𝐀𝐆𝐎 𝐁𝐎𝐓-𝐌𝐃_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4' }, { quoted: m })    
 } catch { 
 let res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`)     
 let json = await res.json() 
 await conn.sendFile(m.chat, json.result.video, '.mp4', `_𝐈𝐃𝐀𝐆𝐎 𝐁𝐎𝐓-𝐌𝐃_`, m)}} 
 } catch (e) { 
